...
logger.Log("INFO", "СГМС (накопичувальна ПР №9) запущено.");
...

{
...

    {
...

        {
            ...
            case "41": DemoGetTopStudents(group); break;

            case "42": DemoAverageGradeByStatus(group); break;

            case "43": DemoStudentsWithHighGrades(group); break;

            case "44": DemoGetUniqueNames(group); break;

            case "45": DemoEsportsAnalytics(); break;
...

{
    ...
    sb.AppendLine("║   СГМС (ЗПФК) — Накопичувальна Практична робота №9       ║");
    ...
    sb.AppendLine("║  --- LINQ (ПР №9) ---                                    ║");

    sb.AppendLine("║  41. GetTopStudents (топ-5 за балом)                     ║");

    sb.AppendLine("║  42. GetAverageGradeByStatus (середній за статусом)      ║");

    sb.AppendLine("║  43. GetStudentsWithHighGrades (відмінники, ≥90)         ║");

    sb.AppendLine("║  44. GetUniqueNames (унікальні ПІБ)                      ║");

    sb.AppendLine("║  45. PlayerStats + EsportsAnalytics (CS2/Dota)           ║");
...
static void DemoGetTopStudents(StudentGroup group)
{
    var top = group.GetTopStudents(5);
    Console.WriteLine("Топ-5 студентів за середнім балом:");
...
static void DemoAverageGradeByStatus(StudentGroup group)
{
    Console.WriteLine("--- Середній бал за кожним статусом ---");
    foreach (StudentStatus status in Enum.GetValues(typeof(StudentStatus)))
    {
        var avg = group.GetAverageGradeByStatus(status);
        Console.WriteLine($"  {status}: {avg:F2}");
    }

    Console.WriteLine();
    Console.WriteLine("1. Перерахувати для обраного статусу  2. Назад");
    Console.Write("Вибір: ");
    if (Console.ReadLine() != "1")
    {
        return;
    ...
    if (int.TryParse(Console.ReadLine(), out var idx) && idx >= 0 && idx <= 3)
    {
        var status = (StudentStatus)idx;
        Console.WriteLine($"Середній бал ({status}): {group.GetAverageGradeByStatus(status):F2}");
    }
}

static void DemoStudentsWithHighGrades(StudentGroup group)
{
    var excellent = group.GetStudentsWithHighGrades(90);
    Console.WriteLine($"Відмінники (бал ≥ 90): {excellent.Count}");
    foreach (var s in excellent)
    {
        Console.WriteLine($"  • {s.FullName}: {s.AverageGrade:F2}");
    }
    Console.WriteLine($"Усі здані лаби ≥ 60: {group.Students.Count(s => s.HasPassedAllLabs())} студ.");
    Console.WriteLine($"Усі студенти з балом ≥ 60: {group.CheckAllStudentsPassed()}");
}

static void DemoGetUniqueNames(StudentGroup group)
{
    var names = group.GetUniqueNames();
    Console.WriteLine($"Унікальних ПІБ: {names.Count}");
    foreach (var name in names)
    {
        Console.WriteLine($"  • {name}");
    }
...
static void DemoEsportsAnalytics()
{
    var players = new List<PlayerStats>
    {
        new() { Nickname = "s1mple_CS2", Kills = 28, Deaths = 12, WinRate = 62.5 },
        new() { Nickname = "ZywOo", Kills = 24, Deaths = 10, WinRate = 58.0 },
        new() { Nickname = "Dendi", Kills = 15, Deaths = 8, WinRate = 55.0 },
        new() { Nickname = "Miracle-", Kills = 22, Deaths = 14, WinRate = 48.0 },
        new() { Nickname = "Puppey", Kills = 5, Deaths = 12, WinRate = 42.0 }
    };

    Console.WriteLine("--- PlayerStats (CS2 / Dota 2) ---");
    foreach (var p in players)
    {
        Console.WriteLine($"  {p.Nickname}: K/D={p.KdRatio:F2}, WinRate={p.WinRate:F1}%, Points={p.TotalPoints}");
    }

    var bestKd = EsportsAnalytics.GetBestKdPlayer(players);
    Console.WriteLine();
    Console.WriteLine($"Найкращий K/D: {bestKd?.Nickname} ({bestKd?.KdRatio:F2})");

    Console.WriteLine();
    Console.WriteLine("--- Групування за WinRate ---");
    foreach (var group in EsportsAnalytics.GroupPlayersByWinRate(players))
    {
        Console.WriteLine($"  {group.Key}: {string.Join(", ", group.Value.Select(p => p.Nickname))}");
    }

    Console.WriteLine();
    Console.WriteLine("--- Сортування за TotalPoints ---");
    foreach (var p in EsportsAnalytics.GetPlayersSortedByPoints(players))
    {
        Console.WriteLine($"  {p.Nickname}: {p.TotalPoints} очок");
    }
}
