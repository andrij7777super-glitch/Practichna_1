...
public class StudentGroup
{
    ...
    public void AddMember(UniversityMember member)
    {
        ...
        if (member is Student addedStudent)
        {
            StudentAdded?.Invoke(addedStudent);
        }
    }
    ...
    public List<Student> GetTopStudents(int count) =>
        _members.OfType<Student>().OrderByDescending(s => s.AverageGrade).Take(count).ToList();

    public double GetAverageGradeByStatus(StudentStatus status) =>
        _members.OfType<Student>()
            .GroupBy(s => s.Status)
            .Where(g => g.Key == status)
            .Select(g => g.Average(s => s.AverageGrade))
            .DefaultIfEmpty(0)
            .First();

    public List<Student> GetStudentsWithHighGrades(double minGrade) =>
        _members.OfType<Student>().Where(s => s.AverageGrade >= minGrade).ToList();

    public List<string> GetUniqueNames() =>
        _members.OfType<Student>().Select(s => s.FullName).Distinct().ToList();

    public bool CheckAllStudentsPassed() =>
        _members.OfType<Student>().All(s => s.AverageGrade >= 60);

    public List<Student> FilterStudents(Predicate<Student> condition) =>
        _members.OfType<Student>().Where(s => condition(s)).ToList();

    public void ProcessStudents(Action<Student> action)
    {
    ...
    public List<TResult> MapStudents<TResult>(Func<Student, TResult> selector) =>
        _members.OfType<Student>().Select(selector).ToList();
